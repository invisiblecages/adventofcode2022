...
internal class Program
{
    static void Main(string[] args)
    {
        ...
        {
            first.Add(input[i].Substring(0, input[i].Length / 2));
            second.Add(input[i].Substring(input[i].Length / 2));
        }

        for (int i = 0; i < first.Count; i++)
        {
            int sum = 0;
            foreach (char ch in first[i])
            {
                if (second[i].Contains(ch))
                    if (char.IsLower(ch))
                    {
                        sum += (int)ch - 96;
                        break;
        ...
        Console.WriteLine("part one: " + nums.Sum());
        nums.Clear();
        ...
        for (int i = 0; i < input.Length; i++)
        {
            ...
            if (partTwo.Count == 3)
            {
                foreach (char ch in partTwo[0])
                {
                    if (partTwo[1].Contains(ch) && partTwo[2].Contains(ch))
                        if (char.IsLower(ch))
                        {
                            sum += (int)ch - 96;
                            break;
        ...
        Console.WriteLine("part two: " + nums.Sum());
    }
}